...
let jsonToProblem json =
    try
        json |> JsonConvert.DeserializeObject<Problem> |> Some
    with _ ->
        None
          ...
          >=> choose
              [ path "/api/solve"
                >=> request (fun req ->
                    ...
                    | Some problem -> OK(sprintf "%s" (problem |> constructProblem |> JsonConvert.SerializeObject))
...
let main argv =
    ...
    let app =
        choose [ apiRoutes; NOT_FOUND "Route not found. Refer to the source code of the api" ]
